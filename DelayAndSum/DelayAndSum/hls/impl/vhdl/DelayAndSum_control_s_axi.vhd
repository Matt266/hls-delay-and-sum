...
architecture behave of DelayAndSum_control_s_axi is
    ...
    constant ADDR_AUTO_RESTART_COUNTER_0  : INTEGER := 16#10#;
    constant ADDR_PHI_DATA_0              : INTEGER := 16#14#;
    constant ADDR_PHI_CTRL                : INTEGER := 16#18#;
    constant ADDR_FC_DATA_0               : INTEGER := 16#1c#;
    constant ADDR_FC_CTRL                 : INTEGER := 16#20#;
    constant ADDR_XPOS1_DATA_0            : INTEGER := 16#24#;
    constant ADDR_XPOS1_CTRL              : INTEGER := 16#28#;
    constant ADDR_XPOS2_DATA_0            : INTEGER := 16#2c#;
    constant ADDR_XPOS2_CTRL              : INTEGER := 16#30#;
    constant ADDR_XPOS3_DATA_0            : INTEGER := 16#34#;
    constant ADDR_XPOS3_CTRL              : INTEGER := 16#38#;
    constant ADDR_XPOS4_DATA_0            : INTEGER := 16#3c#;
    ...
    signal int_auto_restart_counter_0 : UNSIGNED(31 downto 0) := (others => '0');
    signal infnt_auto_restart  : STD_LOGIC;
...
-- ----------------------- Register logic ----------------
        ...
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (w_hs = '1' and waddr = ADDR_AP_CTRL and WSTRB(0) = '1' and WDATA(0) = '1') then
                    ...
                    int_ap_start <= auto_restart_enable; -- clear on handshake/auto restart
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                auto_restart_counter <= to_unsigned(1, 32);
            elsif (ACLK_EN = '1') then
                if ((w_hs = '1' and waddr = ADDR_AUTO_RESTART_COUNTER_0) or (infnt_auto_restart = '1')) then
                    auto_restart_counter <= to_unsigned(1, 32);
                elsif (ap_ready = '1' and auto_restart_enable = '1') then
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_auto_restart_counter_0 <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (int_auto_restart = '1') then
                    int_auto_restart_counter_0 <= (others => '0');
                elsif (w_hs = '1' and waddr = ADDR_AUTO_RESTART_COUNTER_0) then
                    int_auto_restart_counter_0 <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_auto_restart_counter_0);
                end if;
            end if;
        end if;
    end process;
