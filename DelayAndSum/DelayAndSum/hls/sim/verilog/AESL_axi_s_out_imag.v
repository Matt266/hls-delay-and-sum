...
module AESL_axi_s_out_imag (
    ...
    wire TRAN_out_imag_TVALID_temp;
    wire out_imag_TDATA_full;
    wire out_imag_TDATA_empty;
    reg out_imag_TDATA_write_en;
    reg [16 - 1:0] out_imag_TDATA_write_data;
    reg out_imag_TDATA_read_en;
    wire [16 - 1:0] out_imag_TDATA_read_data;
    
    fifo #(2, 16) fifo_out_imag_TDATA (
        .reset(1'b0),
        .write_clock(clk),
        .write_en(out_imag_TDATA_write_en),
        .write_data(out_imag_TDATA_write_data),
        .read_clock(clk),
        .read_en(out_imag_TDATA_read_en),
        .read_data(out_imag_TDATA_read_data),
        .full(out_imag_TDATA_full),
        .empty(out_imag_TDATA_empty));
    
    always @ (*) begin
        out_imag_TDATA_write_en <= TRAN_out_imag_TVALID;
    ...
    reg [31:0] transaction_save_out_imag_TDATA;
    
    assign transaction = transaction_save_out_imag_TDATA;
    
    initial begin : AXI_stream_receiver_out_imag_TDATA
        integer fp;
        reg [16 - 1:0] data;
        reg [8 * 5:1] str;
        
        transaction_save_out_imag_TDATA = 0;
        fifo_out_imag_TDATA.clear();
        wait (reset === 1);
        forever begin
            @ (negedge clk);
            if (done_1 == 1) begin
                fp = $fopen(`TV_OUT_out_imag_TDATA, "a");
                if (fp == 0) begin // Failed to open file
                    $display("ERROR: Failed to open file \"%s\"!", `TV_OUT_out_imag_TDATA);
                    $finish;
                end
                $fdisplay(fp, "[[transaction]] %d", transaction_save_out_imag_TDATA);
                while (~fifo_out_imag_TDATA.empty) begin
                    fifo_out_imag_TDATA.pop(data);
